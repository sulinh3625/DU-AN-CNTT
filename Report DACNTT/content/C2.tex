...
Về nguyên tắc, phản hồi của User có thể là \textbf{hiện (Explicit)} -- điểm số đánh giá trực tiếp như rating 1--5 sao -- hoặc \textbf{ẩn (Implicit)} -- suy ra gián tiếp từ hành vi như lịch sử mua hàng, lượt click \parencite{he2017ncf, rendle2009bpr}. Điểm cần nói rõ ở đây không phải là định nghĩa chung của hai khái niệm này, mà là một ràng buộc thực tế: cả \texttt{DataCoSupplyChainDataset.csv} lẫn \texttt{transactions\_train.csv} của H\&M \textbf{đều không có bất kỳ cột rating nào} -- chỉ có bản ghi giao dịch đã xảy ra. Điều này loại bỏ hoàn toàn khả năng dùng Explicit Feedback, và buộc toàn bộ đề tài phải xây dựng tín hiệu tương tác dưới dạng nhị phân từ chính sự kiện mua hàng:
\begin{equation}
    y_{ui} = \begin{cases}
...
\end{itemize}
Hướng tiếp cận này phù hợp với dữ liệu có tính phiên rõ rệt (streaming, xem video, duyệt web); trên dữ liệu DataCo Supply Chain, tần suất giao dịch của mỗi User thưa và không có mốc thời gian phiên rõ ràng, nên đây được ghi nhận là hướng mở rộng tương lai thay vì lựa chọn chính (Chương~\ref{chap:ketluan}).
...
\begin{equation}
    ...
    \label{eq:lightgcn}
\end{equation}
trong đó $\mathcal{N}_u$ là tập Item lân cận của User $u$ trong đồ thị và $e_u^{*}$ là biểu diễn cuối cùng, tổng hợp có trọng số qua $K$ tầng lan truyền. Đây là hướng phát triển được đề tài xác định là ưu tiên hàng đầu (mục~\ref{sec:huongphattrien}), do dữ liệu chuỗi cung ứng DataCo vốn có độ thưa cao -- bối cảnh mà tín hiệu bậc cao qua đồ thị đã được chứng minh mang lại cải thiện đáng kể so với MF/NCF truyền thống.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|p{3.2cm}|p{2.8cm}|p{2.5cm}|X|}
        ...
        \hline
        \textbf{Kang \& McAuley (2018)} \cite{kang2018self} & SASRec (Self-Attention tuần tự) & Amazon, Steam, MovieLens & Mô hình hóa chuỗi hành vi bằng Self-Attention. \textit{Hạn chế:} cần mốc thời gian/phiên rõ ràng, chưa khai thác được với dữ liệu DataCo. \\
        \hline
        \textbf{Wang et al. (2019); He et al. (2020)} \cite{wang2019ngcf, he2020lightgcn} & NGCF / LightGCN (Graph-based CF) & Gowalla, Amazon-Book, Yelp2018 & Lan truyền tín hiệu tương tác bậc cao qua đồ thị hai phía. \textit{Hạn chế:} chi phí tính toán lan truyền tăng theo số tầng, chưa thử nghiệm trên dữ liệu chuỗi cung ứng. \\
        \hline
        ...
        \hline
        \textbf{Dự án đề xuất (2026)} & \textbf{Mô hình lai NeuMF (Early/Late Fusion)} & \textbf{DataCo Supply Chain} & \textbf{Xây dựng và đánh giá NeuMF, Early Fusion trên dữ liệu chuỗi cung ứng, phản hồi ẩn có trọng số, Ablation Study, đánh giá Popular/Long-tail và kiểm định thống kê đa seed.} \\
        \hline
    \end{tabularx}
\end{table}
