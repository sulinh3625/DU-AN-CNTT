...
\begin{itemize}
    ...
    \item Xây dựng quy trình xử lý dữ liệu hoàn chỉnh trên bộ dữ liệu DataCo Supply Chain \parencite{dataco2019}, biến đổi dữ liệu giao dịch thành ma trận tương tác User--Item dạng tín hiệu ẩn (kết hợp trọng số giá trị đơn hàng), thực hiện lấy mẫu tiêu cực (Negative Sampling) và phân chia tập dữ liệu theo giao thức Leave-One-Out.
...
\textit{Lưu ý về phạm vi:} Dự án tập trung vào nhóm người dùng và sản phẩm đã có đủ lịch sử tương tác để xây dựng biểu diễn Embedding (sau khi lọc $k$-core). Bài toán khởi đầu lạnh (Cold-start) cho người dùng/sản phẩm hoàn toàn mới \textbf{không nằm trong phạm vi đánh giá thực nghiệm} của dự án và được ghi nhận như một hướng phát triển trong tương lai (Chương~\ref{chap:ketluan}).
...
\begin{itemize}
    ...
    \item \textbf{Dữ liệu thực nghiệm:} Bộ dữ liệu DataCo Smart Supply Chain \parencite{dataco2019}, khai thác thông tin khách hàng, sản phẩm và lịch sử giao dịch mua sắm.
...
Điểm xuyên suốt của phương pháp luận là ưu tiên \textbf{tính có thể tái lập và kiểm chứng được}: mọi số liệu trình bày trong báo cáo đều được trích xuất trực tiếp từ các tệp kết quả (\texttt{results\_primary.csv}, \texttt{data\_audit.json}...) do chính pipeline thực nghiệm sinh ra, thay vì ước lượng hay diễn giải gián tiếp.
...
\begin{itemize}
    \item Cung cấp bằng chứng thực nghiệm định lượng, đối chiếu trên \textbf{hai bộ dữ liệu có đặc trưng cấu trúc khác biệt rõ rệt} (DataCo: catalog nhỏ, dày đặc; H\&M: catalog lớn, cực thưa) về việc kiến trúc NeuMF có thực sự vượt trội các nhánh thành phần của chính nó (GMF, MLP) hay không -- một câu hỏi mà phần lớn các công trình gốc chỉ kiểm chứng trên một loại dữ liệu duy nhất (phim ảnh MovieLens, mạng xã hội hình ảnh Pinterest).
